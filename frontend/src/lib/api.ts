...
export const authAPI = {
  ...
  me: async (): Promise<User> => {
    try {
      // Try the auth/user endpoint first
      return await apiGet<User>('auth/user');
    } catch (error) {
      // Fallback to standalone /user endpoint if auth/user fails
      console.warn('auth/user failed, falling back to /user endpoint');
      return await apiGet<User>('user');
    }
  },

  // Get current user (alternative method)
  getCurrentUser: (): Promise<User> => apiGet<User>('user'),
