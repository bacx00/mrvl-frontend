...
function TeamMentions({ teamId, teamName }) {
  ...
  const fetchMentions = async () => {
    try {
      const response = await api.get(`/teams/${teamId}/mentions`);
      const data = response.data?.data || response.data || [];
      setMentions(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Error fetching team mentions:', error);
      setMentions([]);
    } finally {
      setLoading(false);
    }
  ...
  const getContextText = (mention) => {
    switch (mention.context_type) {
      ...
      case 'match_result':
        return `in match vs ${mention.opponent_team}`;
      default:
    ...
    return (
      <div className="text-center py-6">
        <div className="text-gray-400 dark:text-gray-600 text-sm">
          No recent mentions of {teamName}
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                mentioned {teamName} {getContextText(mention)}
              </div>
              {mention.content && (
