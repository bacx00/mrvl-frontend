...
import { hasPermission, hasRole, ROLES } from '../../utils/roleUtils';
...
function ModeratorDashboard({ navigateTo }) {
  ...
  return (
    <RoleThemeProvider user={user}>
      <ModeratorDashboardContent navigateTo={navigateTo} />
    </RoleThemeProvider>
  );
}

function ModeratorDashboardContent({ navigateTo }) {
        ...
        <main className="flex-1 p-6">
          ...
          {activeSection !== 'overview' && activeSection !== 'reports' && (
            <div className="card p-12 text-center">
              ...
              <div className="mt-4">
                <span className="inline-block px-3 py-1 text-sm bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400 rounded-full">
                  Moderator Only
                </span>
              </div>
            </div>
