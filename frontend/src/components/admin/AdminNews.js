  ...
  const fetchNews = async () => {
    try {
      ...
      if (filters.category !== 'all') {
        newsData = newsData.filter(article => (article.category?.slug || article.category?.name || article.category) === filters.category);
      }
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    <span className={`px-2 py-1 text-xs font-bold rounded ${getCategoryColor(article.category?.name || article.category)}`}>
                      {(article.category?.name || article.category || 'NEWS').toUpperCase()}
                    </span>
