  ...
  const performLiveSearch = async (query) => {
    ...
    try {
      ...
      if (forumsRes.status === 'fulfilled') {
        ...
        threads.filter(thread =>
          ...
          results.push({
            ...
            type: 'forum',
            title: thread.title,
            subtitle: `by ${thread.user_name || thread.user?.name || 'Anonymous'}`,
            icon: '',
            data: thread
          });
        });
      ...
      if (newsRes.status === 'fulfilled') {
        const articles = newsRes.value?.data?.data || newsRes.value?.data || [];
        articles.filter(article =>
          article.title?.toLowerCase().includes(searchTerm)
        ).slice(0, 2).forEach(article => {
          results.push({
            id: article.id,
            type: 'news',
            title: article.title,
            subtitle: article.author?.name || 'MRVL Team',
            icon: '',
            data: article
          });
        });
