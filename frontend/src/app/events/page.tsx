    ...
    const fetchEvents = async () => {
      try {
        ...
        const transformedEvents = apiEvents.map((event: any) => ({
          ...
          name: event.name,
          type: event.status === 'ongoing' ? 'ongoing' : 
          ...
          prize: event.prize_pool ? `$${Number(event.prize_pool).toLocaleString()}` : 'TBD',
      ...
      <div className="bg-[#1a242d] border border-[#2b3d4d] rounded overflow-hidden hover:border-[#fa4454] transition-all duration-200 group-hover:shadow-lg">
        <div className="p-4 flex">
          ...
          <div className="mr-4 flex-shrink-0">
            <div className="w-20 h-20 relative bg-[#11161d] rounded overflow-hidden">
              {event.logo || event.image ? (
                <Image
                  src={event.logo || event.image || '/placeholder.png'}
                  alt={event.name}
                  fill
