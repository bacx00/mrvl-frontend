...
export function AuthProvider({ children }: { children: ReactNode }) {
      ...
      if (storedUser && storedToken) {
        ...
        try {
          ...
          if (!currentUser.role && storedUser.role) {
            ...
            console.warn('Role missing from API response, using stored role:', storedUser.role);
          }
          
          // Update storage with the current user data to ensure it's fresh
  ...
  const login = useCallback(async (credentials: LoginCredentials) => {
    try {
      ...
      if (!response.user.role) {
        ...
        console.warn('Role missing from login response, defaulting to user');
      }
  ...
  const register = useCallback(async (data: RegisterData) => {
    try {
      ...
      if (!response.user.role) {
        response.user.role = 'user';
        console.warn('Role missing from register response, defaulting to user');
      }
  ...
  const updateUser = useCallback(async (userData: Partial<User>) => {
    ...
    try {
      ...
      // Preserve role if it's missing from the update response
      if (!updatedUser.role && user.role) {
        updatedUser.role = user.role;
        console.warn('Role missing from update response, preserving existing role:', user.role);
      }
