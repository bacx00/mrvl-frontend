...
export default function MatchClient({ matchId, initialData }: MatchClientProps) {
  ...
  // Real-time updates using HTTP polling for live matches
  useEffect(() => {
    if (!match || match.status !== 'live') return;

    const fetchLiveUpdates = async () => {
      try {
        const response = await fetch(`/api/matches/${matchId}/live-scoreboard`);
        const result = await response.json();
        
        if (result.success) {
          // Update match data with live scores
          setMatch(prevMatch => ({
            ...prevMatch,
            team1_score: result.data.match.team1_score,
            team2_score: result.data.match.team2_score,
            live_stats: {
              ...prevMatch.live_stats,
              current_map: result.data.match.current_map_index + 1,
              timer: result.data.match.timer
            }
          }));
        }
      } catch (error) {
